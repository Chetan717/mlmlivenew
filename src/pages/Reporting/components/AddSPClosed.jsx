...
export default function AddSPClosed({ memberProfile }) {
  ...
  useEffect(() => {
    ...
    const fetchPlans = async () => {
      try {
        ...
        // Fetch all SP Closed entries to get already-closed plan IDs
        ...
        // Exclude plans that already have an SP Closed entry
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      batch.forEach((item) => {
        const ref = doc(collection(db, "reportsp"));
        wb.set(ref, {
      ...
      await wb.commit();
      toast.success(`${batch.length} SP Closed submitted`);
      setBatch([]);
      setOpen(false);
    } catch {
  ...
  return (
    <SectionCard
      ...
      title="Add SP Closed"
      subtitle="Record SP (Sales Point) closed"
      count={batch.length}
      open={open}
      setOpen={setOpen}
    >
