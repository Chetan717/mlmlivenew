...
export default function AddKitBook({ memberProfile, onClose }) {
  const [plans, setPlans] = useState([]);
  const [selectedPlan, setSelectedPlan] = useState(null);
  const [batch, setBatch] = useState([]);
  const [form, setForm] = useState({ name: "", mobile: "", address: "", amount: "" });
  ...
  const handleAdd = () => {
    ...
    if (!form.amount.trim()) { toast.danger("Amount is required"); return; }
    setBatch((prev) => [...prev, { ...form, planId: selectedPlan.id, _id: Date.now() }]);
    setForm({ name: "", mobile: "", address: "", amount: "" });
    setSelectedPlan(null);
  };

  const handleSubmit = async () => {
    if (batch.length === 0) { toast.danger("Add at least one kit book"); return; }
    setSubmitting(true);
    try {
      const wb = writeBatch(db);
      const now = Timestamp.now();
      batch.forEach((item) => {
        const ref = doc(collection(db, COLLECTIONS.REPORTKITBOOK));
        wb.set(ref, {
      ...
      await wb.commit();
      toast.success(`${batch.length} kit book(s) saved`);
      setBatch([]);
      if (onClose) onClose();
    } catch {
  ...
  return (
    <>
      ...
      <ModalActions onAdd={handleAdd} onSubmit={handleSubmit} onCancel={onClose} count={batch.length} submitting={submitting} label="Kit Books" />
    </>
  );
}
