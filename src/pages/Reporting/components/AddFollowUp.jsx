...
export default function AddFollowUp({ memberProfile, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await wb.commit();
      toast.success(`${batch.length} follow-up(s) saved`);
      setBatch([]);
      if (onClose) onClose();
    } catch {
  ...
  return (
    <>
      ...
      <div>
        <label className="text-[10px] font-semibold text-muted-foreground block mb-1">Followup Note</label>
        <textarea
      ...
      <ModalActions onAdd={handleAdd} onSubmit={handleSubmit} onCancel={onClose} count={batch.length} submitting={submitting} label="Follow-Ups" />
    </>
  );
}
